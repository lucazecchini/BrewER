...
def gold_generation(raw_path_1, raw_path_2, gold_path, specifications):
    ...
    ds = ds[ds['left_spec_id'].isin(specifications['id'].tolist()) &
            ds['right_spec_id'].isin(specifications['id'].tolist())]
    ds.to_csv(gold_path, index=False)


def blocking(ds_dict):
    # Generate blocks
    ds_name = 'altosight_sigmod_no_nan'
...
def main():
    ...
    ds_path = "data/altosight_sigmod_no_nan_dataset.csv"
    ...
    gold_path = "data/altosight_sigmod_no_nan_gold.csv"

    # Generate the ground truth
    ds = pd.read_csv('data/altosight_sigmod_no_nan_dataset.csv')
    gold_generation(raw_path_1, raw_path_2, gold_path, ds)
